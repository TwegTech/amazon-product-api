...
class AmazonScraper {
    ...
    httpRequest({ uri, method, qs, json, body, form }) {
        ...
        //adding functionality for arabic asins
        if(uri == "https://www.amazon.ae/s"){
            uri="https://www.amazon.ae";
        }
        //https://www.amazon.ae/s is a 302 permanent redirect and cannot fetch anything. if still does not work then i will comment out the rejection in catch statement.
        if (this.lang == 'ar' && !uri.includes('?')) {
            uri = uri + '?language=ar_AE';
        } else if (this.lang == 'en' && !uri.includes('?')) {
            uri = uri + '?language=en_AE';
        } else if (this.lang == 'ar' && uri.includes('?')) {
            uri = uri + '&language=ar_AE';
        } else if (this.lang == 'en' && uri.includes('?')) {
            uri = uri + '&language=en_AE';
        }
        console.log(uri); //adding for testing why products are failing
